...
@cli.command()
def list_todos(username:str):
    """
    Output each todo's ID, text, username, and completion status.
    """
    with get_session() as db:
        user = db.exec(select(User).where(User.username == username)).one_or_none()
        if not user:
...
@cli.command()
def mark_all_done(username:str):
    with get_session() as db:
        user = db.exec(select(User).where(User.username == username)).one_or_none()
        if not user:
            print("User doesn't exist")
        ...
        db.commit()
        print("All todos marked as done")
